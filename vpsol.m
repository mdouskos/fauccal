...
%   vanish points coordinates

% Least square solution with the use of vanish line
sigc=inf;
SYN=0;
                        ...
                        case 1
                            ...
                            for k=1:size(horl,2)
                                if horl(j+mtr2,k)~=0
                                    ...
                                    fcv=fcvp(xv2(i),yv2(i),inlp2,ina2(i,j+mtr2),inc2(i,j+mtr2),pntx,pnty);
                                    A(mtr,1)=fcv(1);
                                    if lenxv==4
                                        A(mtr,5:6)=fcv(2:3);
                                        A(mtr,6+lnum)=fcv(4);
                    ...
                    for j=1:(lena(i)+lenc(i))
                        if lenxv==4
                            if (j<=length(hia) && ina(i,hia(j))~=0)
                                ina2(i,hia(j))=ina(i,hia(j))+dx(6+j);
                            elseif (j<=length(hic) && inc(i,hic(j))~=0)
                                inc2(i,hic(j))=inc(i,hic(j))+dx(6+j);
                            end
                        else
                            if (j<=length(hia) && ina(i,j)~=0)
                                ina2(i,hia(j))=ina(i,hia(j))+dx(5+j);
                            elseif (j<=length(hic) && inc(i,j)~=0 )
                                inc2(i,hic(j))=inc(i,hic(j))+dx(5+j);
                            end
                        end
                    end
                end
    ...
    if ( SYN==0 && isfinite(xv(i)))
        ...
        while term==0
            ...
            if i==1
                ...
                A=zeros(obsnv-minobs,lnv+2);
                dl=zeros(obsnv-minobs,1);
                r=(obsnv-minobs)-(lnv+2);
            end
            mtr=1;
            lnum=1;
            for j=1:(lena(i)+lenc(i))
                    ...
                    case 1
                        ...
                        for k=1:size(horl,2)
                            if horl(j,k)~=0
                                pntx=impoint00(impoint00(:,4)==horl(j+mtr2,k),5);
                                pnty=impoint00(impoint00(:,4)==horl(j+mtr2,k),6);
                                fcv=fcvp2(xv2(i),yv2(i),ina2(i,j+mtr2),inc2(i,j+mtr2),pntx,pnty);
                                A(mtr,1:2)=fcv(1:2);
                                A(mtr,2+lnum)=fcv(3);
                                dl(mtr,1)=-fcv(4);
                                mtr=mtr+1;
            ...
            if term~=1
                ...
                for j=1:(lena(i)+lenc(i))
                    if (j<=length(hia) && ina(i,hia(j))~=0)
                        ina2(i,hia(j))=ina(i,hia(j))+dx(2+j);
                    elseif (j<=length(hia) && inc(i,j)~=0)
                        inc2(i,hic(j))=inc(i,hic(j))+dx(2+j);
                    end
                end
            end
        end
